...
## What is ERCFund?
ERCFund makes it possible to invest into a multitude of actively managed ERC20-Tokens and Ether by introducing a dynamically minted and burned token as the medium for shares in the fund. Fund managers can freely trade with these tokens and hopefully make profits. Based on the assets under management the price for one token should be continously updated.

To buy a share in the fund one simply has to send Ether to the fund and in exchange the individual will receive "shares" of the fund in the form of fund-tokens based on the current price. These tokens can also be sold at any time by calling the withdraw function of the fund which sends the corresponding value of Ether back to a specified wallet.
